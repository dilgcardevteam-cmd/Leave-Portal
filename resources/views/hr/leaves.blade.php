                        ...
                        <div class="flex flex-col gap-4">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="hidden sm:flex items-center gap-1 bg-gray-100 p-1 rounded-lg">
                                    ...
                                    <a href="{{ route('hr.leaves', array_filter(['status'=>null,'q'=>$q])) }}" class="px-3 py-1.5 rounded-md text-sm {{ request('status') ? 'text-gray-600' : 'bg-white shadow text-gray-900' }}">All</a>
                                    <a href="{{ route('hr.leaves', array_filter(['status'=>'pending','q'=>$q])) }}" class="px-3 py-1.5 rounded-md text-sm {{ request('status')==='pending' ? 'bg-yellow-500 text-white shadow' : 'text-gray-600' }}">Pending</a>
                                    <a href="{{ route('hr.leaves', array_filter(['status'=>'approved','q'=>$q])) }}" class="px-3 py-1.5 rounded-md text-sm {{ request('status')==='approved' ? 'bg-green-600 text-white shadow' : 'text-gray-600' }}">Approved</a>
                                    <a href="{{ route('hr.leaves', array_filter(['status'=>'rejected','q'=>$q])) }}" class="px-3 py-1.5 rounded-md text-sm {{ request('status')==='rejected' ? 'bg-red-600 text-white shadow' : 'text-gray-600' }}">Rejected</a>
                                </div>
                            </div>
                            <form id="hrFilterForm" method="GET" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                <div class="relative flex-1">
                                    ...
                                    <input type="text" name="q" id="hrSearchInput" value="{{ request('q') }}" placeholder="Search by name, email, category, date…" class="pl-10 border-gray-300 rounded-lg shadow-sm w-full" />
                                </div>
                            </form>
                        </div>
                        <script>
                                ...
                                <tbody class="bg-white divide-y divide-gray-100">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <a href="{{ route('hr.leaves.show', $leave) }}" class="inline-flex items-center gap-2 px-3 py-1.5 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 shadow-sm">
