                ...
                <section class="p-4 sm:p-6 lg:p-8">
                    ...
                    <div class="bg-white p-6 shadow sm:rounded-xl border border-gray-200">
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <h2 class="text-xl font-semibold text-gray-800">ARD Panel</h2>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('ard.leaves') }}" class="px-3 py-2 bg-gray-100 border border-gray-200 text-gray-800 rounded-md hover:bg-gray-200">Back</a>
                            </div>
                        </div>
                        <div class="border-b mb-4">
                            <nav class="flex gap-2" role="tablist" aria-label="ARD tabs">
                                <button type="button" class="ard-tab px-4 py-2 rounded-t-md bg-indigo-50 text-indigo-700 border border-b-0 border-indigo-200" data-tab="req">LEAVE REQUEST</button>
                                <button type="button" class="ard-tab px-4 py-2 rounded-t-md text-gray-600 hover:text-gray-800 border border-b-0 border-transparent" data-tab="credits">CERTIFICATION OF LEAVE CREDITS</button>
                                <button type="button" class="ard-tab px-4 py-2 rounded-t-md text-gray-600 hover:text-gray-800 border border-b-0 border-transparent" data-tab="reco">RECOMMENDATION</button>
                                <button type="button" class="ard-tab px-4 py-2 rounded-t-md text-gray-600 hover:text-gray-800 border border-b-0 border-transparent" data-tab="decision">APPROVED FOR / DISAPPROVED DUE TO</button>
                            </nav>
                        </div>
                        ...
                        <div id="ard-tab-req">
                            ...
                            <div class="border-t mt-6 pt-6 flex justify-end">
                                <button type="button" id="ardNext1" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Next</button>
                            </div>
                        </div>
                        <div id="ard-tab-credits" class="hidden">
                            ...
                            <div class="mt-6 flex items-center justify-between">
                                ...
                                <button type="button" id="ardNext2" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Next</button>
                            </div>
                        </div>
                        <div id="ard-tab-reco" class="hidden">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-3">
                                    @php 
                                        $ar = $leave->details_json['ard_recommendation'] ?? []; 
                                        $seed = !empty($ar) ? $ar : ($leave->details_json['dc_recommendation'] ?? []);
                                    @endphp
                                    <label class="flex items-center gap-2">
                                        <input type="checkbox" class="rounded text-indigo-600 focus:ring-indigo-500" data-group="reco" data-toggle="#ard_reco_for_approval" {{ (!empty($seed) && ($seed['decision'] ?? '')==='approval') ? 'checked' : '' }} disabled>
                                        <span class="text-sm text-gray-700">For approval</span>
                                    </label>
                                    <div id="ard_reco_for_approval" class="{{ (!empty($seed) && ($seed['decision'] ?? '')==='approval') ? '' : 'hidden' }}">
                                    ...
                                    <label class="flex items-center gap-2">
                                        <input type="checkbox" class="rounded text-indigo-600 focus:ring-indigo-500" data-group="reco" data-toggle="#ard_reco_disapproval" {{ (!empty($seed) && ($seed['decision'] ?? '')==='disapproval') ? 'checked' : '' }} disabled>
                                        <span class="text-sm text-gray-700">For disapproval due to</span>
                                    </label>
                                    <div id="ard_reco_disapproval" class="{{ (!empty($seed) && ($seed['decision'] ?? '')==='disapproval') ? '' : 'hidden' }}">
                                ...
                                <div class="mt-6 flex items-center justify-between">
                                    ...
                                    <button type="button" id="ardRecoNext" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Next</button>
                                </div>
                        </div>
                        <div id="ard-tab-decision" class="hidden">
                            <div class="mt-6 flex items-center justify-between">
                                @php $finalLocked = $leave->workflow_state !== 'final_pending'; @endphp
                                <form action="{{ route('ard.leaves.status', $leave) }}" method="POST" class="flex flex-wrap items-center gap-3">
                                    @csrf
                                    <select name="status" class="border-gray-300 rounded-md shadow-sm" @disabled($finalLocked)>
                                    ...
                                    <input type="text" name="comment" class="border-gray-300 rounded-md shadow-sm w-full md:w-72" placeholder="Comment (optional)" @disabled($finalLocked)>
                                    <button class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed" @disabled($finalLocked)>Finalize Decision</button>
                                </form>
                                @if($finalLocked)
                                    <div class="text-sm text-gray-500">Final decision is locked for this request.</div>
                                @endif
                            </div>
                        </div>
                    </div>
...
(function(){
    ...
    const tabs = document.querySelectorAll('.ard-tab');
    const panels = {
        req: document.getElementById('ard-tab-req'),
        credits: document.getElementById('ard-tab-credits'),
        reco: document.getElementById('ard-tab-reco'),
        decision: document.getElementById('ard-tab-decision'),
    };
    ...
    const btn1 = document.getElementById('ardNext1');
    const btn2 = document.getElementById('ardNext2');
    if(btn1){
    ...
    const btn3 = document.getElementById('ardRecoNext');
    if(btn3){
