<aside class="hr-sidebar hidden min-h-[calc(100vh-4rem)] bg-gradient-to-b from-[#1c2a5c] to-[#17254f] p-4 text-white lg:block">
    ...
    <nav class="space-y-2" aria-label="DC sidebar">
        <a href="{{ route('dc.index') }}" class="flex items-center rounded-xl py-3 text-lg transition @if(request()->routeIs('dc.index')) bg-white/10 shadow-inner @else hover:bg-white/10 @endif focus:outline-none focus:ring-0" :class="sidebarCollapsed ? 'justify-center px-3' : 'gap-3 px-4'" @if(request()->routeIs('dc.index')) aria-current="page" @endif title="Dashboard">
        ...
        <a href="{{ route('dc.leaves') }}" class="flex items-center rounded-xl py-3 text-lg transition @if(request()->routeIs('dc.leaves*')) bg-white/10 shadow-inner @else hover:bg-white/10 @endif focus:outline-none focus:ring-0" :class="sidebarCollapsed ? 'justify-center px-3' : 'gap-3 px-4'" @if(request()->routeIs('dc.leaves*')) aria-current="page" @endif title="Leave Requests">
        ...
        <a href="{{ route('dc.downloads') }}" class="flex items-center rounded-xl py-3 text-lg transition @if(request()->routeIs('dc.downloads')) bg-white/10 shadow-inner @else hover:bg-white/10 @endif focus:outline-none focus:ring-0" :class="sidebarCollapsed ? 'justify-center px-3' : 'gap-3 px-4'" @if(request()->routeIs('dc.downloads')) aria-current="page" @endif title="Download">
            <svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M5 20h14v-2H5v2zm7-18l-5.5 5.5h3.5V15h4V7.5H17.5L12 2z"/></svg>
            <span x-show="!sidebarCollapsed" class="whitespace-nowrap">Download</span>
        </a>
        <a href="{{ route('dc.help') }}" class="flex items-center rounded-xl py-3 text-lg transition @if(request()->routeIs('dc.help')) bg-white/10 shadow-inner @else hover:bg-white/10 @endif focus:outline-none focus:ring-0" :class="sidebarCollapsed ? 'justify-center px-3' : 'gap-3 px-4'" @if(request()->routeIs('dc.help')) aria-current="page" @endif title="Help & Support">
