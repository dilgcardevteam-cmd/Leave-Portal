                ...
                <section class="p-4 sm:p-6 lg:p-8">
                    <div class="bg-white p-6 shadow sm:rounded-xl border border-gray-200">
                        ...
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <h2 class="text-xl font-semibold text-gray-800">DC Panel</h2>
                                <p class="text-sm text-gray-500">Review forwarded request and provide recommendation.</p>
                                @if($leave->hr_approved_at)
                                <div class="mt-2 text-xs text-gray-600">
                                    HR approved on <time datetime="{{ $leave->hr_approved_at?->toIso8601String() }}" data-realtime="absolute">
                                        {{ $leave->hr_approved_at?->timezone(config('app.timezone'))->format('M d, Y g:i A') }}
                                    </time>
                                    @if($leave->hr_comment)
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('dc.leaves') }}" class="px-3 py-2 bg-gray-100 border border-gray-200 text-gray-800 rounded-md hover:bg-gray-200">Back</a>
                            </div>
                        </div>
                        <div class="border-b mb-4">
                            <nav class="flex gap-2" role="tablist" aria-label="DC tabs">
                                <button type="button" class="lgmed-tab px-4 py-2 rounded-t-md bg-indigo-50 text-indigo-700 border border-b-0 border-indigo-200" data-tab="req">LEAVE REQUEST</button>
                                <button type="button" class="lgmed-tab px-4 py-2 rounded-t-md text-gray-600 hover:text-gray-800 border border-b-0 border-transparent" data-tab="credits">CERTIFICATION OF LEAVE CREDITS</button>
                                <button type="button" class="lgmed-tab px-4 py-2 rounded-t-md text-gray-600 hover:text-gray-800 border border-b-0 border-transparent" data-tab="reco">RECOMMENDATION</button>
                            </nav>
                        </div>
                        <div id="lgmed-tab-req">
                            ...
                            <div class="border-t mt-6 pt-6 flex justify-end">
                                <button type="button" id="lgmedNext1" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Next</button>
                            </div>
                        </div>
                        <div id="lgmed-tab-credits" class="hidden">
                            ...
                            <div class="mt-6 flex items-center justify-between">
                                ...
                                <button type="button" id="lgmedNext2" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Next</button>
                            </div>
                        </div>
                        <div id="lgmed-tab-reco" class="hidden">
                            <form method="POST" action="{{ route('dc.leaves.recommendation', $leave) }}">
                                @csrf
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-3">
                                    @php $dr = $leave->details_json['dc_recommendation'] ?? []; @endphp
                                    <label class="flex items-center gap-2">
                                        <input type="checkbox" name="for_approval" value="1" class="rounded text-indigo-600 focus:ring-indigo-500" data-group="reco" data-toggle="#reco_for_approval" {{ (!empty($dr) && ($dr['decision'] ?? '')==='approval') ? 'checked' : '' }}>
                                        <span class="text-sm text-gray-700">For approval</span>
                                    </label>
                                    <div id="reco_for_approval" class="{{ (!empty($dr) && ($dr['decision'] ?? '')==='approval') ? '' : 'hidden' }}">
                                        <input type="text" name="approval_remarks" class="mt-1 w-full border-gray-300 rounded-md shadow-sm" placeholder="Remarks (optional)" value="{{ old('approval_remarks', $dr['remarks'] ?? '') }}" {{ (!empty($dr) && ($dr['decision'] ?? '')==='approval') ? '' : 'disabled' }}>
                                    </div>
                                    <label class="flex items-center gap-2">
                                        <input type="checkbox" name="for_disapproval" value="1" class="rounded text-indigo-600 focus:ring-indigo-500" data-group="reco" data-toggle="#reco_disapproval" {{ (!empty($dr) && ($dr['decision'] ?? '')==='disapproval') ? 'checked' : '' }}>
                                        <span class="text-sm text-gray-700">For disapproval due to</span>
                                    </label>
                                    <div id="reco_disapproval" class="{{ (!empty($dr) && ($dr['decision'] ?? '')==='disapproval') ? '' : 'hidden' }}">
                                        <textarea name="disapproval_reason" rows="3" class="mt-1 w-full border-gray-300 rounded-md shadow-sm" placeholder="Specify reasons" {{ (!empty($dr) && ($dr['decision'] ?? '')==='disapproval') ? '' : 'disabled' }}>{{ old('disapproval_reason', $dr['reason'] ?? '') }}</textarea>
                                    </div>
                                </div>
                                </div>
                                <div class="mt-6 flex items-center justify-between">
                                    <div class="text-sm text-gray-600">
                                        <span class="font-medium">Encoded by:</span>
                                        {{ $dr['meta']['updated_by_name'] ?? '—' }}
                                        @if(!empty($dr['meta']['updated_at']))
                                            <span class="text-gray-500">on</span>
                                            <time datetime="{{ $dr['meta']['updated_at'] }}" data-realtime="absolute">{{ \Carbon\Carbon::parse($dr['meta']['updated_at'])->timezone(config('app.timezone'))->format('M d, Y g:i A') }}</time>
                                            <span class="ml-1 text-gray-500" data-realtime="relative" datetime="{{ $dr['meta']['updated_at'] }}"></span>
                                        @endif
                                    </div>
                            ...
                            <div class="mt-8 border-t pt-6">
                                <form id="dcFinalizeForm" method="POST" action="{{ route('dc.leaves.status', $leave) }}" class="flex flex-wrap items-center gap-3">
                                    @csrf
                                    <input type="hidden" name="reco_for_approval" id="dc_reco_for_approval">
                                    <input type="hidden" name="reco_for_disapproval" id="dc_reco_for_disapproval">
                                    <input type="hidden" name="reco_approval_remarks" id="dc_reco_approval_remarks">
                                    <input type="hidden" name="reco_disapproval_reason" id="dc_reco_disapproval_reason">
                                    <select name="status" class="border-gray-300 rounded-md shadow-sm">
                                        <option value="approved">Forward to Final Approval</option>
                                        <option value="rejected">Reject at DC Stage</option>
                                    </select>
                                    <input type="text" name="comment" class="border-gray-300 rounded-md shadow-sm w-full md:w-72" placeholder="Comment (optional)">
                                    <button class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Finalize DC Decision</button>
                                </form>
                            </div>
                        </div>
...
(function(){
    ...
    const tabs = document.querySelectorAll('.lgmed-tab');
    const panels = {
        req: document.getElementById('lgmed-tab-req'),
        credits: document.getElementById('lgmed-tab-credits'),
        reco: document.getElementById('lgmed-tab-reco'),
    };
    ...
    @if (session('active_tab') === 'reco')
        const b = document.querySelector('.lgmed-tab[data-tab="reco"]');
        if (b) { setActive(b); show('reco'); }
    @endif
    ...
    const btn1 = document.getElementById('lgmedNext1');
    const btn2 = document.getElementById('lgmedNext2');
    if(btn1){
    ...
    const finalize = document.getElementById('dcFinalizeForm');
    if(finalize){
        finalize.addEventListener('submit', ()=>{
            const forApproval = document.querySelector('input[name=\"for_approval\"]');
            const forDisapproval = document.querySelector('input[name=\"for_disapproval\"]');
            const approvalRemarks = document.querySelector('input[name=\"approval_remarks\"]');
            const disapprovalReason = document.querySelector('textarea[name=\"disapproval_reason\"]');
            document.getElementById('dc_reco_for_approval').value = forApproval && forApproval.checked ? '1' : '0';
            document.getElementById('dc_reco_for_disapproval').value = forDisapproval && forDisapproval.checked ? '1' : '0';
            document.getElementById('dc_reco_approval_remarks').value = approvalRemarks ? approvalRemarks.value : '';
            document.getElementById('dc_reco_disapproval_reason').value = disapprovalReason ? disapprovalReason.value : '';
        });
    }
})();
</script>
